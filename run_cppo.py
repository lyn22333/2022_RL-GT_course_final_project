...
class CPPO_Runner(object):
    ...
    def __init__(self, args, env):
        ...
        self.log_dir = args['log_dir'] + '/CPPO' + '_' + str(self.n_agents)
        # self.log_dir = args['log_dir'] + '/CPPO'
        if self.use_beta:
            self.log_dir += '_Beta'
        ...
        self.policy = Policy(self.args,
                            actor_obs_space=self.cent_obs_space,
        ...
        self.buffer = SharedReplayBuffer(self.args,
                                        1, #环境的agent有多个，实际只有一个Central agent
                                        actor_obs_space=self.cent_obs_space,
                                        critic_obs_space = self.cent_obs_space,
    ...
    def run(self):
        for iteration in range(self.iterations):
            ...
            for step in range(self.episode_length):
                ...
                actions_env = np.reshape(actions, (self.num_envs, self.n_agents, -1))
                # (n_agents, num_envs, -1)
                actions_env = np.swapaxes(actions_env, 0, 1)
                ...
                rewards = np.reshape(rewards, (self.num_envs, 1, 1))

                dones = np.expand_dims(dones.numpy(),1).repeat(1, 1)
    ...
    def warmup(self):
        ...
        self.buffer.actor_obs[0] = share_obs.copy()
    ...
    def insert(self, data):
        ...
        masks = np.ones((self.num_envs, 1, 1), dtype=np.float32)
...
shape = (action_space.shape[0]*env.n_agents,)
low = np.array([-1.0 for _ in range(shape[0])])
high = np.array([1.0 for _ in range(shape[0])])
cent_action_space = Box(low, high, shape, np.float64)
arg = {
    "num_envs": 300,
    "n_agents": env.n_agents,
    "obs_space": obs_space,
    "action_space": cent_action_space,
    "iterations": 400,
    "episode_length": 200,
